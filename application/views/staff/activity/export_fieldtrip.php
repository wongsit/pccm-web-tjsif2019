<?php
  ...
  $fileName = 'TJSIF2019_'.$title.'.xlsx';
  ...
  //กำหนดรายละเอียดเกี่ยวกับเอกสาร
  $objPHPExcel->getActiveSheet()->SetCellValue('A2', 'All Field trip member: '.count($users).' Total Checked: '.count($check));
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue('J3', 'DateTime');
  ...
  foreach ($check as $user) {
      ...
      $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $user->update_time);
      $rowCount++;
      $i++;
  }
  $rowCount+=1;
  $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, 'Date: '.$date_time_stamp.' Developer by: Griffinics');     //ที่
  //savefile
