<?php
  ...
  $fileName = 'TJSIF2019_'.$title.'_'.$date_stamp.'.xlsx';
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue('A1', 'TJSIF2019 '.$title.'. Date: '.$date_time_stamp);
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue('P3', 'Food Other');
  $objPHPExcel->getActiveSheet()->SetCellValue('Q3', 'Position');
  $objPHPExcel->getActiveSheet()->SetCellValue('R3', 'fieldtrip');
  ...
  foreach ($users as $user) {
      ...
      $objPHPExcel->getActiveSheet()->SetCellValue('P' . $rowCount, $user->food_other);
      $objPHPExcel->getActiveSheet()->SetCellValue('Q' . $rowCount, $user->position);
      $objPHPExcel->getActiveSheet()->SetCellValue('R' . $rowCount, $this->Users_model->fetch_fieldtrip_name($user->trip)); //ชั้น
      $rowCount++;
      $i++;
  }
  $rowCount+=1;
