...
class Organization extends Users_Controller
{
   public function __construct(){
 	...
 	public function index()
 	{
      ...
      $data['orgs'] = $this->Org_model->fetch_org();  // //ดึงข้อมูลจาก tb_org ทั้งหมด
 			...
 			$data['page'] = 'member/organization/index';
  ...
  public function info($org_id)
  {
     ...
     $data['countrys'] = $this->Org_model->fetch_country();
      ...
      $data['page'] = 'member/organization/info';
      $this->load->view('templates/tjsif_member',$data);   //แสดง view
  }
    ...
    if($this->Site_model->check_register_date($this->System_model->get_date()) == 0){   //ถ้าเกินกำหนด หรือไม่เปิดลงทะเบียน
      $this->session->set_flashdata(
        ...
        $data['my_ip'] = $this->System_model->get_client_ip(); //ไอพี
         ...
         $data['page'] = 'member/organization/new';
         $this->load->view('templates/tjsif_member',$data);   //แสดง view
    }else{
      //เช็คการอนุญาต (premission) ของ user อัตโนมัติจาก Users_Controller
      $data['org_types'] = $this->Org_model->fetch_org_type(); //ดึงข้อมูลจาก tb_org_type ทั้งหมด
    ...
    if ($this->input->server('REQUEST_METHOD') == TRUE) {
      ...
      if ($this->form_validation->run() == TRUE) {    //ถ้าการตรวจสอบ rule ถูกต้อง จะได้ TRUE
        if($this->Org_model->add_org($_POST) != null){
          ...
          redirect('members/organization/index/');
        }else{
          //แสดงผลลัพท์จากระบบ
          $this->session->set_flashdata(
            array(
  ...
  public function edit($org_id)
  {
     ...
     $data['my_ip'] = $this->System_model->get_client_ip(); //ไอพี
      $data['title'] = 'Organization';
      $data['page'] = 'member/organization/edit';
      $this->load->view('templates/tjsif_member',$data);   //แสดง view
  }
    ...
    if ($this->input->server('REQUEST_METHOD') == TRUE) {
      ...
      if ($this->form_validation->run() == TRUE) {    //ถ้าการตรวจสอบ rule ถูกต้อง จะได้ TRUE
        if($this->Org_model->update_org2($_POST) != null){
          ...
          redirect('members/organization/info/'.$this->input->post('id'));
        }else{
          //แสดงผลลัพท์จากระบบ
          $this->session->set_flashdata(
            array(
  ...
  function upload_picture()
  {
    ...
    $data['org'] = $this->Org_model-> fetch_org_data($this->session->userdata('org_id'));  // //ดึงข้อมูลจาก tb_org ทั้งหมด
    $data['org_types'] = $this->Org_model->fetch_org_type(); //ดึงข้อมูลจาก tb_org_type ทั้งหมด
    $data['countrys'] = $this->Org_model->fetch_country();
    $data['my_ip'] = $this->System_model->get_client_ip(); //ไอพี
    $data['page'] = 'member/organization/upload_picture';        //หน้าเพจที่จะแสดง
  ...
  function do_upload()
  {
    ...
    $config['file_name']           = 'tjsif2019-org-'.$this->session->userdata('org_id');    //กำหนดชื่อไฟล์ที่ต้องการ
    ...
    else
    {
            ...
            redirect('members/organization/info/'.$this->session->userdata('org_id'));
    }
  }
  ...
  function create_thumb(){
    ...
    $config['source_image'] = './assets/images/orgs/tjsif2019-org-'.$this->session->userdata('org_id').'.jpg';
